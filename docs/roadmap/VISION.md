...
In the long term, device-neutral pattern recognition may turn user-confirmed historical hypotheses into virtual energy assets. These knowledge assets can help prediction and optimization anticipate recurring or conditional loads without pretending that aggregate measurements prove a device identity.

A first-class Simulation Framework will make the architecture reproducible and explorable through replay, accelerated time, scenario benchmarks, demonstrations, synthetic data, and regression validation. Live and simulated operation should reuse production components so optimization strategies can be compared objectively without teaching the optimizer about execution mode.
