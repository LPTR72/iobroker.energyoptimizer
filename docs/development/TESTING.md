...
- History Service domain foundation collection and aggregation for power, energy counters, state of charge, strictly validated binary state, temperature, price, generic numbers, invalid samples, every aggregation level through `1d`, rolling 24-hour boundaries, local calendar-day boundaries, daylight-saving transitions, and resolution-chain guards
- `TimeSeriesMerger`
